 ...
 * @author VNPT
 */
public class CityWorkWater {

    String url;
    String user;
    String pass;
    int timeout;
    Logger logger = LogManager.getLogger(CityWorkWater.class.getName());
    String token;
    HttpClient HTTP_CLIENT;

    public CityWorkWater(String url, String user, String pass, int timeout) {
        this.url = url;
        this.user = user;
        this.pass = pass;
...
//            HttpClient HTTP_CLIENT = HttpClientBuilder.create().setDefaultRequestConfig(config).build();
            logger.info("getCustomerInfo: " + url + "/api/vnptpayment/laythongtinkhachhang?token=" + token + "&maKhachHang=" + customerCode);
            HttpGet request = new HttpGet(url + "/api/vnptpayment/laythongtinkhachhang?token=" + token + "&maKhachHang=" + customerCode);
...
//            HttpClient HTTP_CLIENT = HttpClientBuilder.create().setDefaultRequestConfig(config).build();
        ...
        public BillInfoHbFloat getBillInfoF(String customerCode, ResponseCode resCode) {
        try {
//            RequestConfig config = RequestConfig.custom()
//                    .setConnectTimeout(timeout * 1000)
//                    .setConnectionRequestTimeout(timeout * 1000)
...
//            HttpClient HTTP_CLIENT = HttpClientBuilder.create().setDefaultRequestConfig(config).build();
            ...
            logger.info("getBillInfo output: " + output.toString());
            BillInfoHbFloat res = new Gson().fromJson(output.toString(), BillInfoHbFloat.class);
